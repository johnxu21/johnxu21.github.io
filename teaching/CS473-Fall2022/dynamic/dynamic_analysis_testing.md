...
* [DSpot](https://github.com/STAMP-project/dspot) is a tool for test-amplification in Java code. Test-amplification generates new test cases using the original tests as seeds. It supports projects build in Maven and Gradle, and it has plugins for IntelliJ and Eclipse.
* [Randoop](https://randoop.github.io/randoop/) is a test generation tool. It automatically generates unit tests for Java classes.

Setup / Preparation
==============
Be sure to follow the setup and the tasks from the last session (Refactoring Assistants). Especially, the task on executing SonarQube. As per usual, you will also need IntelliJ and JPacman. Moreover, if you have not already, download the book for this course "Object-Oriented Reengineering Patterns
...
We will begin by using the IntelliJ IDE test coverage plugin. The testing and coverage plugins should be enabled by default. If you are not sure, check if your plugins called "Coverage", "JUnit", and "TestNG" are enabled (inside the Preferences you can see the plugins).
 
First, make sure that you can test your JPacman, by using the following command line in the IntelliJ terminal:
...
For the second task, we will increase the coverage on JPacman. Doing that is very simple, we just need to write more tests. In this task, we are going to write one new test case. As you saw in the last task, the coverage for several packages is zero percent.
...
./gradlew sonarqube -Dsonar.projectKey=jpacman  
    -Dsonar.login=admin -Dsonar.password=reengineering
