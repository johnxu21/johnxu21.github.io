...
## Use case 2 - Example 3: Software Configuration Management
...
developer aligns with a `Software Configuration Management (SCM)` task. Specifically, the developer is providing specification for writing a Continuous Integration (CI) pipeline using GitHub Actions to enforce certain rules regarding the merging of pull requests based on the presence of specific labels.
<!-- **Program synthesis** is the automated process of generating code that satisfies
a given specification or set of constraints, emphasizing the derivation of functional properties
of the code [(Hou et al., 2023)](https://arxiv.org/pdf/2308.10620.pdf). -->
